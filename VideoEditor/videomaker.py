...
def prepare_background(reddit_id,length,W, H, duration):
    my_config = config.load_config()

    video = VideoFileClip(my_config['Background']['path']).without_audio()
    vide_duration = duration
...
def make_final_video(
    ...
    background_clip = prepare_background(reddit_id, length,W,H, duration)
    ...
    audio_clips = [AudioFileClip(title_audio_path), AudioFileClip(body_audio_path)]
    ...
    body = ImageClip(body_image_path).set_duration(audio_clips[1].duration).set_opacity(new_opacity).set_position("center")
    resized_body = resize(body, width=screenshot_width)
    image_clips.append(
        resized_body,
    )
